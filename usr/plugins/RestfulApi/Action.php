...
class RestfulApi_Action extends Typecho_Widget implements Widget_Interface_Do
{
    ...
    public function __construct($request, $response, $params = null)
    {
        ...
        $this->config = $this->options->plugin('RestfulApi');
    }
    ...
    public function postsAction()
    {
        ...
        $select = $this->db
            ->select('cid', 'title', 'created', 'modified', 'type')
            ->from('table.contents')
            ->where('type = ?', 'domain')
            ->where('status = ?', 'sale')
            ->where('created < ?', time())
        ...
        if (isset($cids)) {
            ...
            $select->where('title LIKE ? OR description LIKE ?', $searchQuery, $searchQuery);
        }
    ...
    public function usersAction()
    {
        ...
        $users = array();
        foreach ($result as $key => $value) {
            $postSelector = $this->db->select('cid', 'title', 'created', 'modified', 'type')
                ->from('table.contents')
                ->where('status = ?', 'sale')
                ->where('type = ?', 'domain')
    ...
    public function archivesAction()
    {
        ...
        $select = $this->db->select('cid', 'title', 'created', 'modified', 'type', 'description')
            ->from('table.contents')
            ->where('status = ?', 'sale')
            ->where('type = ?', 'domain')
    ...
    public function upgradeAction()
    {
        ...
        $remotePluginContent = file_get_contents('https://raw.githubusercontent.com/moefront/typecho-plugin-RestfulApi/master/Plugin.php');
        $remoteActionContent = file_get_contents('https://raw.githubusercontent.com/moefront/typecho-plugin-RestfulApi/master/Action.php');
