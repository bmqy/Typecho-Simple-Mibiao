...
<?php \Widget\Metas\Platform\Cloud::alloc('sort=count&desc=1&limit=200')->to($platforms); ?>
...
$(document).ready(function() {
    ...
    // platform autocomplete 提示
    var platforms = $('#platforms'), platformsPre = [];

    if (platforms.length > 0) {
        var items = platforms.val().split(','), result = [];
        for (var i = 0; i < items.length; i ++) {
            var platform = items[i];
        ...
        ?>, {
            tokenLimit: 1,
            propertyToSearch:   'platforms',
            tokenValue      :   'platforms',
            searchDelay     :   0,
            preventDuplicates   :   true,
            animateDropdown :   false,
            hintText        :   '<?php _e('请输入平台名'); ?>',
            noResultsText   :   '<?php _e('此平台不存在, 按回车创建'); ?>',
            prePopulate     :   platformsPre,
        ...
        // platform autocomplete 提示宽度设置
        $('#token-input-platforms').focus(function() {
            var t = $('.token-input-dropdown'),
                offset = t.outerWidth() - t.width();
            t.width($('.token-input-list').outerWidth() - offset);
        });
