...
class Stat extends Base
{
     ...
     * @return integer
     */
    protected function ___salePostsNum(): int
    {
        return $this->db->fetchObject($this->db->select(['COUNT(cid)' => 'num'])
            ->from('table.contents')
            ->where('table.contents.type = ?', 'domain')
            ->where('table.contents.status = ?', 'sale'))->num;
    }
     ...
     * @return integer
     */
    protected function ___soldPostsNum(): int
    {
        return $this->db->fetchObject($this->db->select(['COUNT(cid)' => 'num'])
            ->from('table.contents')
            ->where('table.contents.type = ?', 'domain')
            ->where('table.contents.status = ?', 'sold'))->num;
    }
     ...
     * @return integer
     */
    protected function ___notPostsNum(): int
    {
        return $this->db->fetchObject($this->db->select(['COUNT(cid)' => 'num'])
            ->from('table.contents')
            ->where('table.contents.type = ? OR table.contents.type = ?', 'domain', 'domain_draft')
            ->where('table.contents.status = ?', 'not'))->num;
    }
     ...
     * @return integer
     */
    protected function ___plaformsNum(): int
    {
        return $this->db->fetchObject($this->db->select(['COUNT(mid)' => 'num'])
            ->from('table.metas')
            ->where('table.metas.type = ?', 'plaform'))->num;
    }
}
