 ...
 * @package Widget
 */
class Archive extends ContentsDomain
{
    /**
     * 调用的风格文件
     *
    ...
    public function execute()
    {
        ...
        if (!$selectPlugged) {
            if ($this->parameter->preview) {
                ...
                if ('domain' == $this->parameter->type || 'post' == $this->parameter->type || 'page' == $this->parameter->type) {
                    if ($this->user->hasLogin()) {
                        if (isset($this->parameter->status)) {
                    ...
                    if ($this->user->hasLogin()) {
                        if (isset($this->parameter->status)) {
                            ...
                            $select = $this->select()->where(
                                '(table.contents.status = ? OR table.contents.status = ? OR table.contents.status = ?) AND table.contents.authorId = ?',
                                'sale',
                                'sold',
                                'not',
                                $this->user->uid
        ...
        $order = $this->parameter->order==='ASC' ? Db::SORT_ASC : Db::SORT_DESC;
        $select->order('table.contents.'. $this->parameter->orderby, $order)
...
(function () {
    ...
    {
        $select->where('table.contents.type = ?', 'domain');
