 ...
 */
class ContentsDomain extends Base implements QueryInterface
{
    /**
     * 获取查询对象
     *
     ...
     * 输出文章平台
     *
     * @param string $split 多个平台之间分隔符
     * @param boolean $link 是否输出链接
     * @param string|null $default 如果没有则输出
     */
    public function platforms(string $split = ',', bool $link = true, ?string $default = null)
    {
        /** 取出platforms */
        if ($this->platforms) {
            $result = [];
            foreach ($this->platforms as $platform) {
                $result[] = $link ? '<a href="' . $platform['permalink'] . '">'
                    . $platform['name'] . '</a>' : $platform['name'];
            }
     ...
     * @throws Exception
     */
    public function totalPriceByStatus(string $status = ''): int
    {
        $sql = $this->db->select('SUM(price) AS totalPrice')->from('table.contents');
        if ($status != '') {
            $sql->where('table.contents.status = ?', $status);
        }
        $sql->where('table.contents.type = ?', 'domain', [Metas::alloc(), 'filter']);
        $totalPrice = $this->db->query($sql)->fetch()['totalPrice'];
