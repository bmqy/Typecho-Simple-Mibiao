...
class Metas extends Base implements QueryInterface
{
     ...
     * 根据platform获取ID
     *
     * @param mixed $inputPlatforms 平台名
     * @return array|int
     * @throws Exception
     */
    public function scanPlatforms($inputPlatforms)
    {
        $platforms = is_array($inputPlatforms) ? $inputPlatforms : [$inputPlatforms];
        $result = [];
        ...
        foreach ($platforms as $platform) {
            ...
            $row = $this->db->fetchRow($this->select()
                ->where('type = ?', 'platform')
                ->where('name = ?', $platform)->limit(1));

            if ($row) {
                $result[] = $row['mid'];
            } else {
                $slug = Common::slugName($platform);
        ...
        return is_array($inputPlatforms) ? $result : current($result);
    }
    ...
    {
        // 取出count为0的标签
        $platforms = array_column($this->db->fetchAll($this->db->select('mid')
            ->from('table.metas')->where('type = ? AND count = ?', 'platforms', 0)), 'mid');

        foreach ($platforms as $platform) {
            // 确认是否已经没有关联了
            $content = $this->db->fetchRow($this->db->select('cid')
                ->from('table.relationships')->where('mid = ?', $platform)
                ->limit(1));
     ...
     * @throws Exception
     */
    public function refreshCountByTypeAndStatus(int $mid, string $type, string $status = 'sale')
    {
