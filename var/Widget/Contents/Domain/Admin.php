 ...
 */
class Admin extends ContentsDomain
{
    /**
     * 用于计算数值的语句对象
     *
    ...
    public function getMenuTitle(): string
    {
        if (isset($this->request->uid)) {
            return _t('%s的域名', $this->db->fetchObject($this->db->select('screenName')->from('table.users')
    ...
    public function execute()
    {
        ...
        /** 如果具有编辑以上权限,可以查看所有域名,反之只能查看自己的域名 */
        if (!$this->user->pass('editor', true)) {
            $select->where('table.contents.authorId = ?', $this->user->uid);
        } else {
            if ('on' == $this->request->__typecho_all_domains) {
                Cookie::set('__typecho_all_domains', 'on');
            } else {
                if ('off' == $this->request->__typecho_all_domains) {
                    Cookie::set('__typecho_all_domains', 'off');
                }

                if ('on' != Cookie::get('__typecho_all_domains')) {
        ...
        /** 按状态查询 */
        if ('not' == $this->request->status) {
            $select->where('table.contents.type = ? OR table.contents.status = ?', 'post_draft', "not");
        } elseif ('sale' == $this->request->status) {
            $select->where(
        ...
        }  elseif ('sold' == $this->request->status) {
            $select->where(
                '(table.contents.type = ? OR table.contents.type = ?) AND table.contents.status = ?',
                'domain',
                'post_draft',
