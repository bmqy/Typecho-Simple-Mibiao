 ...
 */
class Edit extends ContentsDomain implements ActionInterface
{
    /**
     * 自定义字段的hook名称
     *
    ...
    public function execute()
    {
        ...
        if (!empty($this->request->cid)) {
            $this->db->fetchRow($this->select()
                ->where('table.contents.type = ? OR table.contents.type = ?', 'domain', 'domain_draft')
                ->where('table.contents.cid = ?', $this->request->filter('int')->cid)
                ->limit(1), [$this, 'push']);

            if ('domain_draft' == $this->type && $this->parent) {
                $this->response->redirect(
                    Common::url('write-domain.php?cid=' . $this->parent, $this->options->adminUrl)
                );
            }

            if (!$this->have()) {
                throw new Exception(_t('域名不存在'), 404);
    ...
    {
        if ('domain' == $value['type'] || 'page' == $value['type']) {
            $draft = $this->db->fetchRow(ContentsDomain::alloc()->select()
                ->where(
                    'table.contents.type = ?',
                    $value['cid'],
                    $value['type'] . '_draft'
    ...
    public function getFieldItems(): array
    {
        ...
        if ($this->have()) {
            ...
            foreach ($rows as $row) {
                $isFieldReadOnly = ContentsDomain::pluginHandle()
    ...
    public function writePost()
    {
        ...
        if ($this->request->is('do=publish')) {
            ...
            /** 设置提示信息 */
            Notice::alloc()->set('domain' == $this->type ?
                _t('域名 "<a href="%s">%s</a>" 已经发布', $this->permalink, $this->title) :
                _t('域名 "%s" 等待审核', $this->title), 'success');
            ...
            /** 页面跳转 */
            $this->response->redirect(Common::url('manage-domains.php?' . $pageQuery, $this->options->adminUrl));
        } else {
            /** 保存域名 */
            $contents['type'] = 'domain_draft';
            $this->save($contents);
            ...
            if ($this->request->isAjax()) {
                ...
                /** 返回原页面 */
                $this->response->redirect(Common::url('write-domain.php?cid=' . $this->cid, $this->options->adminUrl));
            }
        }
    }
    ...
    protected function publish(array $contents)
    {
        ...
        /** 是否是从草稿状态发布 */
        $isDraftToPublish = ('domain_draft' == $this->type || 'page_draft' == $this->type);

        $isBeforePublish = ('not' != $this->status);
        $isAfterPublish = ('not' != $contents['status']);
        ...
        if ($realId > 0) {
            /** 插入平台 */
            if (array_key_exists('platforms', $contents)) {
                $this->setPlatforms($realId, $contents['platforms'], !$isDraftToPublish && $isBeforePublish, $isAfterPublish);
            }
     ...
     * @throws DbException
     */
    public function setPlatforms(int $cid, ?string $platforms, bool $beforeCount = true, bool $afterCount = true)
    {
        $platforms = str_replace('，', ',', $platforms);
        $platforms = array_unique(array_map('trim', explode(',', $platforms)));
        $platforms = array_filter($platforms, [Validate::class, 'xssCheck']);

        /** 取出已有tag */
        $existPlatforms = array_column(
            $this->db->fetchAll(
                $this->db->select('table.metas.mid')
                ->from('table.metas')
                ->join('table.relationships', 'table.relationships.mid = table.metas.mid')
                ->where('table.relationships.cid = ?', $cid)
                ->where('table.metas.type = ?', 'platform')
            ),
            'mid'
        );
        ...
        /** 删除已有platform */
        if ($existPlatforms) {
            foreach ($existPlatforms as $platform) {
        ...
        /** 取出插入平台 */
        $insertPlatforms = Metas::alloc()->scanPlatforms($platforms);

        /** 插入平台 */
        if ($insertPlatforms) {
            foreach ($insertPlatforms as $platform) {
                if (0 == strlen($platform)) {
                    continue;
                }
    ...
    protected function getPageOffsetQuery(int $cid, ?string $status = null): string
    {
        return 'page=' . $this->getPageOffset(
            ...
            'domain',
            $status,
            'on' == $this->request->__typecho_all_domains ? 0 : $this->user->uid
        );
    }
    ...
    public function markPost()
    {
        ...
        foreach ($posts as $post) {
            ...
            $postObject = $this->db->fetchObject($this->db->select('status', 'type')
                ->from('table.contents')->where('cid = ? AND (type = ? OR type = ?)', $post, 'domain', 'domain_draft'));
            ...
            if ($this->isWriteable(clone $condition) && count((array)$postObject)) {
                ...
                // 刷新Metas
                if ($postObject->type == 'domain') {
                    $op = null;

                    if ($status == 'sale' && $postObject->status != 'sale') {
                        $op = '+';
                    } elseif ($status != 'sale' && $postObject->status == 'sale') {
                        $op = '-';
                    }
                    ...
                    if (!empty($op)) {
                        ...
                        foreach ($metas as $meta) {
                            $this->db->query($this->db->update('table.metas')
                                ...
                                ->where('mid = ? AND (type = ? OR type = ?)', $meta['mid'], 'platform', 'tag'));
                        }
                    }
                }
                ...
                $draft = $this->db->fetchRow($this->db->select('cid')
                    ->from('table.contents')
                    ->where('table.contents.parent = ? AND table.contents.type = ?', $post, 'domain_draft')
                    ->limit(1));
        ...
        Notice::alloc()
            ->set(
                $markCount > 0 ? _t('域名已经被标记为<strong>%s</strong>', $statusList[$status]) : _t('没有域名被标记'),
    ...
    public function deletePost()
    {
        ...
        foreach ($posts as $post) {
            ...
            if ($this->isWriteable(clone $condition) && count((array)$postObject) && $this->delete($condition)) {
                ...
                /** 删除分类 */
                $this->setCategories($post, [], 'sale' == $postObject->status
                    && 'domain' == $postObject->type);

                /** 删除标签 */
                $this->setTags($post, null, 'sale' == $postObject->status
                    && 'domain' == $postObject->type);
        ...
        /** 设置提示信息 */
        Notice::alloc()->set(
            $deleteCount > 0 ? _t('域名已经被删除') : _t('没有域名被删除'),
     ...
     * @throws DbException
     */
    protected function ___platforms(): array
    {
        if ($this->have()) {
            return $this->db->fetchAll($this->db
                ->select()->from('table.metas')
                ->join('table.relationships', 'table.relationships.mid = table.metas.mid')
                ->where('table.relationships.cid = ?', $this->cid)
                ->where('table.metas.type = ?', 'platform'), [Metas::alloc(), 'filter']);
        }
    ...
    {
        if ($this->have()) {
            if ('domain_draft' == $this->type || 'page_draft' == $this->type) {
                return $this->row;
            } else {
                return $this->db->fetchRow(ContentsDomain::alloc()->select()
                    ->where(
                        'table.contents.type = ? OR table.contents.type = ?',
                        'domain_draft',
                        'page_draft'
                    )
                    ->limit(1), [ContentsDomain::alloc(), 'filter']);
            }
        }
